...
func (config *config) setupCommonFlags() {
	...
	// flag del fork: canale NDJSON su stdout per l'app Electron
	flag.BoolVar(
		&config.NDJSON,
		"ndjson",
		false,
		"Emit every decoded payload to stdout as NDJSON (logs move to stderr).",
	)

	// flag del fork: processo figlio senza systray né auto-updater
	flag.BoolVar(
		&config.Headless,
		"headless",
		false,
		"Run without systray and without self-updater (managed as a child process).",
	)
}
...
func (config *config) setupLogs() {
	...
	// in modalità NDJSON stdout è riservato al canale dati: i log vanno su stderr
	terminalWriter := colorable.NewColorableStdout()
	if config.NDJSON {
		terminalWriter = colorable.NewColorableStderr()
	}
	...
	if err == nil {
		...
		multiWriter := io.MultiWriter(terminalWriter, strippedFileWriter)
		log.SetOutput(multiWriter)
	} else {
		log.SetOutput(terminalWriter)
