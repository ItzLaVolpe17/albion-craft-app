...
func decodeEvent(params map[uint8]interface{}) (event operation, err error) {
	...
	// fork: decoder personali (emessi solo in locale via NDJSON)
	case evNewCharacter:
		event = &eventNewCharacterLocal{}
	case evHarvestFinished:
		event = &eventHarvestFinished{}
	case evUpdateFame:
		event = &eventUpdateFame{}
	case evHealthUpdate:
		event = &eventHealthUpdate{}
	default:
		return nil, nil
	}
